...
- **front-facing workstations**: a 2×1 desk block (`desk_<variant>_l/r`,
  aluminum monitor back toward the viewer) with the agent's spawn point
  exactly on the block's **top edge** — the bust stays fully visible, the
  desk covers the lap; the nameplate chip is pushed onto the desk front
  via `theme.agentLabelOffsetY` (per-agent `labelOffsetY` for deeper
  desks); the example uses a **4-row vertical pitch** between stacked
  workstations so status badges never crowd the next nameplate;
- **one shared floor texture** (no zone carpets) — furniture contrasts
  through its own colors (dark espresso desks, aluminum monitors);
- tall wall furniture (vending machine, water cooler) spans the two wall
  face rows so it stands **flush against the wall** (feet on the wall base
